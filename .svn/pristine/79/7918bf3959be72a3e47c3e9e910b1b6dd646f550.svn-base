...
@interface  ZSWSDataCollectionView ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,JWCollectionViewCellDelegate,TZImagePickerControllerDelegate,ZSActionSheetViewDelegate>
...
@implementation ZSWSDataCollectionView
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.addDataStyle == ZSAddResourceDataCountless)
    {
        if (indexPath.row < array.count) {
            ...
            //            cell.layer.borderColor=RGB(222).CGColor;
        ...
        if (self.addDataStyle == ZSAddResourceDataTwo){
            ...
            if (!self.isShowAdd) {
                ...
                if (!(fileModel.dataUrl && indexPath.row == 1)) {
                    cell.nameLabel.text = indexPath.row == 0 ? fileModel.subCategory : @"";
                }
            }
        }
        //如果是关闭或者完成，不给加号
...
//        }
    ...
    else
    {
        //2.1两张
        if (self.addDataStyle != ZSAddResourceDataCountless) {
            selectModel = currentArray[self.currentIndexPath.row];//不是两个加号的情况 加号的model属性赋值
            [currentArray replaceObjectAtIndex:self.currentIndexPath.row withObject:fileModel];//替换原有model,用于照片回显
            [self setModelAttributeWithFileModel:fileModel Model:selectModel isFormAbum:NO];//点击加号的时候model重新赋值
        }
        //2.1多张照片
        else {
            [currentArray addObject:fileModel];//替换原有model,用于照片回显
            [self setModelAttributeWithFileModel:fileModel Model:selectModel isFormAbum:NO];//点击加号的时候model重新赋值
        }
...
- (void)GoToPhotoLibraryWithDataStyle:(ZSAddResourceDataStyle)dataStyle
{
    ...
    NSInteger maxImageCount = (dataStyle == ZSAddResourceDataCountless && !weakSelf.bool_isFromBanker) ? 9 : 1;
...
#pragma mark 原图
                [assets enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    [[TZImageManager manager]getOriginalPhotoWithAsset:imageAssets newCompletion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                        ...
                        //文件model赋值
                        ZSWSFileCollectionModel *fileModel = [weakSelf configureImageModel:photo imageData:nil];
                        //5.1收款银行卡/还款银行卡两项资料
                        if (weakSelf.bool_isFromBanker)
                        {
                            [currentArray addObject:fileModel];//新增model,用于照片回显
                            //将需要上传的照片存入到数组当中,上传成功再从数组中删除
                            [self.needUploadArray addObject:fileModel];
                        ...
                        //5.2其余资料
                            ...
                            if (weakSelf.addDataStyle == ZSAddResourceDataCountless) {
                                ...
                                [currentArray replaceObjectAtIndex:weakSelf.currentIndexPath.row withObject:fileModel];//替换原有model,用于照片回显
                                [weakSelf setModelAttributeWithFileModel:fileModel Model:selectModel isFormAbum:YES];//点击加号的时候model的属性重新赋值
                            }
                            //将需要上传的照片存入到数组当中,上传成功再从数组中删除
                            [self.needUploadArray addObject:fileModel];
                            //图片上传获取url
...
- (void)getProductSaleLoanByUploadFileModel:(ZSWSFileCollectionModel *)fileModel data:(NSData *)data isFromBanker:(BOOL)isFromBanker
{
     ...
     [ZSRequestManager uploadImageWithNativeAPI:data SuccessBlock:^(NSDictionary *dic)
     {
         ...
         //3.如果是银行卡上传,识别一下
         if (isFromBanker == YES) {
             [self getBankInfoByUploadFileModel:fileModel];
         }
...
- (void)preViewBigImages
{
    ...
    photoBroseView.modelsURL = self.preViewImages;//模型数组赋值
...
#pragma mark JWCollectionViewCellDelegate 删除资料代理
- (void)deleteDataByIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (!self.bool_isFromBanker) {
        ...
        if (!fileModel.image && !fileModel.imageData && fileModel.isNewImage == NO) {
            [self.deletdataIDArray addObject:fileModel.tid];
        }
    }
