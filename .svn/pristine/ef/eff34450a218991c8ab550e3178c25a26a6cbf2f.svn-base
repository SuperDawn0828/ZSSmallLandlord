...
@interface PYPhotoView () <UIActionSheetDelegate, UIGestureRecognizerDelegate,UMSocialShareMenuViewDelegate,ZSActionSheetViewDelegate>
/** 单击手势 */
...
#pragma mark 记录预览时的最原始大小（未伸缩\旋转）
    ...
    if (longPress.state == UIGestureRecognizerStateBegan) {  // 长按结束
        ...
        UIAlertAction *savePhotoAction = [UIAlertAction actionWithTitle:@"保存到相册" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action)
        {
            ...
            if(author == ALAuthorizationStatusRestricted || author ==ALAuthorizationStatusDenied){
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"请前往设置->小房主金福->照片 开启相册权限" message:@"" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                [alert show];
            }else {
        ...
        //2.分享按钮
        UIAlertAction *sharePhotoAction = [UIAlertAction actionWithTitle:@"分享" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action)
        {
            //判断是否安装微信
            BOOL hadInstalledWeixin = [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"weixin://"]];
            BOOL hadInstalledQQ     = [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"mqq://"]];
            if (!hadInstalledWeixin && !hadInstalledQQ){
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"请先安装微信或QQ" message:@"" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                [alert show];
            }else{
                [self shareAPP:self.image];
            }
        }];
        //3.取消按钮
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:nil];
        [alertController addAction:savePhotoAction];
        [alertController addAction:sharePhotoAction];
...
#pragma mark PYAcitonSheetDeleagate(保存图片到本地)
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    if (error) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"保存失败" message:@"" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
        [alert show];
    }
    else
    {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"保存成功" message:@"" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
        [alert show];
    }        
}

#pragma mark 分享
- (void)shareAPP:(UIImage *)webpageImage
{
    //设置平台
    ...
    [UMSocialUIManager showShareMenuViewInWindowWithPlatformSelectionBlock:^(UMSocialPlatformType platformType, NSDictionary *userInfo) {
        ...
        imgview.image = webpageImage;
        //        [imgview sd_setImageWithURL:[NSURL URLWithString:webpageUrl]];
        [shareObject setShareImage:imgview.image];
        //分享消息对象设置分享内容对象
...
#pragma mark 监听滚动，判断cell是否在屏幕上，初始化cell
...
#pragma mark /** 根据手势触摸点修改相应的锚点，就是沿着触摸点做相应的手势操作 */
