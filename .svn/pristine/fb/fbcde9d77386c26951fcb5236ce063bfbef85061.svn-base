//
//  ZSBankHomePersonListCell.m
//  ZSSmallLandlord
//
//  Created by 黄曼文 on 2017/8/3.
//  Copyright © 2017年 黄曼文. All rights reserved.
//

#import "ZSBankHomePersonListCell.h"

@implementation ZSBankHomePersonListCell
...
- (void)initViews
{
    ...
    //未反馈图片
    self.imgview = [[UIImageView alloc]initWithFrame:CGRectMake(ZSWIDTH-95, (70-23)/2, 95, 23)];
    self.imgview.image = [UIImage imageNamed:@"bar_feedback_s"];
    ...
    if (detailModel.isBankCredit.intValue == 1) {
        if (detailModel.creditResult) {
            if (detailModel.creditResult.intValue == 0) {
                self.label_feedbackState.hidden = YES;
                self.timeLabel.hidden = YES;
                if (detailModel.isNotFeedback) {
                    self.imgview.hidden = NO;//未反馈的人显示"点击立即反馈"图片
                }else{
                    self.imgview.hidden = YES;//已经反馈过的订单不显示了
                }
            }
            else if (detailModel.creditResult.intValue == 1)
            {
                self.label_feedbackState.hidden = NO;
            ...
            self.label_feedbackState.hidden = NO;
            self.label_feedbackState.textColor = ZSColorAllNotice;
