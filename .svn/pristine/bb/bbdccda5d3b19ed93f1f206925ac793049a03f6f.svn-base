...
@interface IDPhoto ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
//        int rects[32];
        ...
        if (idInfo !=nil && [idInfo isOK]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.backgroundView.hidden = YES;
                target.view.userInteractionEnabled = YES;
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"无法识别该图片，请手动输入身份证信息" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alert show];
            });
        }
#endif
