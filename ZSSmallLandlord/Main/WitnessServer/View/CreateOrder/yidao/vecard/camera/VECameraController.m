...
#import "VECameraController.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import "VEContextManager.h"
#import "sys/sysctl.h"
...
@interface VECameraController () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate,AVCaptureMetadataOutputObjectsDelegate>
{
    BOOL bFirstTime;
    BOOL bNotLessThan5s;
}
...
@implementation VECameraController
...
- (BOOL)setupSession:(NSError **)error {
    ...
    self.eaglContext = [VEContextManager sharedInstance].eaglContext;
    self.ciContext = [VEContextManager sharedInstance].ciContext;
...
- (void)startRecording {
    dispatch_async([self globalQueue], ^{
        if (![self isRecording]) {
            ...
            self.assetWriterVideoInput.transform = VETransformForDeviceOrientation(orientation);
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if ([captureOutput isEqual:self.videoDataOutput]) {
        ...
        if(bInProcessing == NO)
        {
            ...
            //            VeCardInfo * veInfo = [self doRec:imageBuffer];
            //            if (veInfo !=nil )
            //            {
            //                [self.recDelegate VECardRecognited:veInfo];
            //            }
        }
    }
}
...
CGAffineTransform VETransformForDeviceOrientation(UIDeviceOrientation orientation) {
...
CGRect getVEGuideFrame( CGRect rect)
{
...
CGRect combinRectVE(CGRect A, CGRect B)
{
    CGFloat t,b,l,r;
...
- (VeCardInfo * )doRec:(CVImageBufferRef)imageBuffer
{
    @synchronized(self)
    {
...
//            size_t cbCrPitch = NSSwapBigIntToHost(planar->componentInfoCbCr.rowBytes);
            ...
            //            int ret = EXCARDS_RecoVeLicDataST(pixelAddress, (int)width, (int)height, (int)rowBytes, (int)8, 0, &exveCard);
            int ret = EXVECardRecoNV12STV2(pixelAddress, cbCrBuffer, (int)width, (int)height, 1, &exveCard);
            
            if (ret < 0){
                ZSLOG(@"VECardRecApi，ret[%d]", ret);
            }else{
                bHasResult = YES;
                ZSLOG(@"ret=[%d]", ret);
