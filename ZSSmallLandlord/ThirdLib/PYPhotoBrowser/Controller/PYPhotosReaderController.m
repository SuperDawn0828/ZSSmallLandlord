...
- (UILabel *)pageLabel
{
    if (!_pageLabel) {
        ...
        pageLabel.textColor = ZSColorWhite;
...
- (UILabel *)label_message
{
    if (!_label_message) {
        ...
        messageView.py_height = 35;
        messageView.py_width = self.view.py_width;
        messageView.py_y = self.view.py_height - 35;
        messageView.backgroundColor = ZSColorBlack;
        messageView.alpha = 0.7;
        [self.view addSubview:messageView];
        //内容
        UILabel *messageLabel = [[UILabel alloc] init];
        messageLabel.py_height = 35;
        messageLabel.py_width = self.view.py_width - 30;
        messageLabel.py_y = self.view.py_height - 35;
        messageLabel.py_x = 15;
        messageLabel.font = [UIFont boldSystemFontOfSize:13];
        messageLabel.textColor = ZSColorWhite;
        _label_message = messageLabel;
    }
...
- (void)showPhotosToWindow:(PYPhotoBrowseView *)window
{
    ...
    window.hidden = NO;
    window.backgroundColor = ZSColorBlack;
    ...
    // 显示图片上传人和上传上传时间
    [self fillInDatatOfUploadMessage];
}

#pragma mark 图片上传相关信息
- (void)fillInDatatOfUploadMessage
{
    ZSWSFileCollectionModel *model = self.window.modelsURL[self.pageControl.currentPage];
    if (model.createByName.length > 0 || model.createDate.length > 0) {
        self.label_message.text = NSStringFormat(@"%@上传于%@",SafeStr(model.createByName) ,SafeStr(model.createDate));
        [self.view addSubview:self.label_message];
        return;
    }
    else
    {
...
- (void)deviceOrientationDidChange
{
    ...
    tempWindow.backgroundColor = ZSColorBlack;
    // 记录刚开始的旋转
