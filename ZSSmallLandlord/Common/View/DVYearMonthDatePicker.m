...
- (NSDate *)date
{
    ...
    NSDictionary *DateDic =@{@"time":dateStr,
                             };
    [NOTI_CENTER postNotification:[NSNotification notificationWithName:@"timeDate" object:nil userInfo:DateDic]];
...
- (void)selectToday
{
    [self selectRow: self.todayIndexPath.row-6
        inComponent: MONTH
           animated: NO];

    NSInteger yearsint = 0;
    [self selectRow: yearsint
        inComponent: YEAR
           animated: NO];
}
...
- (void)loadDefaultsParameters
{
    ...
    //最大年限
    self.maxYear   = [ZSTool getCurrentYear] + 10;
    self.rowHeight = 44;
